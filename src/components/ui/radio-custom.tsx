...
const DEFAULTS = {
  ...
  outlineTransition: "border-color 0.2s ease-in-out",
  outlineHoverColor: "hsl(var(--ring))",
    ...
    >
      ...
      >
        <motion.path
          ...
          animate={
            checked ? { pathLength: 1, opacity: 1 } : { pathLength: 0, opacity: 0 }
          }
    ...
    return (
      ...
      >
        {options.map((opt, idx) => {
          ...
          const uniqueId = opt.id || `${stableId}-${idx}`;
          ...
          return (
            <label
              ...
              onClick={(e) => {
                if (!isDisabled) {
                  e.preventDefault();
                  // Radio button logic: only one can be selected
            ...
            >
              <SingleCheckbox
                ...
                borderRadius="50%" // Make it a circle for radio
              />
            </label>
          );
        })}
